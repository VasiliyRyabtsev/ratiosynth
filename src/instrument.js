 ...
 *
 * This is what a string or a column of air does, and it is also where just
 * intonation comes from — 3/2 sounds smooth because the third harmonic of the
 * lower note lands on the second harmonic of the upper one. So an instrument
 * built on this list is already in agreement with ordinary ratios.
 */
 ...
 *
 * Real strings do this — stiffness pushes the upper partials sharp, which is
 * why piano tuning is stretched. Here it also serves as the knob that breaks
 * the agreement with ordinary ratios on purpose, when perfect agreement turns
 * out to be too clean.
 *
 * Amount 0 leaves the list alone. This is the one place we step outside exact
 * ratios, so it returns plain multipliers rather than ratios.
 */
 ...
 *
 * Two of the four aliveness rules live here:
 *
 * - Each partial gets its own decay time, shorter as you go up. Real objects
 *   lose their high partials first; a sound where everything fades together
 *   is instantly recognisable as synthetic.
 * - Each partial gets its own small detuning, a few cents either way, fixed
 *   per instrument. Perfectly aligned partials fuse into something sterile.
 *   (The moving part of the drift happens per note, in the worklet.)
 *
 * The other two — the noise burst at the attack, and louder meaning brighter —
 * are properties of how a note is struck, so they live in the worklet too.
 */
 ...
 *
 * Sorted by frequency, because that is what the roughness sweep expects.
 */
