...
import { fromFraction, format, UNISON, THREE_OVER_TWO, FIVE_OVER_FOUR, equals } from "../src/ratio.js";
...
test("the facts are just the facts", () => {
  const sonority = new Sonority();
  sonority.noteOn(1, THREE_OVER_TWO, { velocity: 0.8, at: 0 });
  sonority.noteOn(2, FIVE_OVER_FOUR, { velocity: 0.4, at: 0.5 });
...
test("a single note is its own centre", () => {
  const sonority = new Sonority();
  sonority.noteOn(1, THREE_OVER_TWO, { at: 0 });
...
test("the same triad moved keeps its shape and moves its centre", () => {
  ...
  const reading = sonority.read(0);
  assert.ok(equals(reading.centre, THREE_OVER_TWO), `got ${format(reading.centre)}`);
});

test("a sounding note is strongly favoured as the centre", () => {
  // Play only the 5/4 and 3/2 of a triad on 1/1. The absent root is a
  ...
  // for itself, which no empty point can match. So 3/2 takes it.
  const sonority = new Sonority();
...
test("an unoccupied point wins only when gravity puts it there", () => {
  ...
  assert.ok(equals(free.read(0).centre, THREE_OVER_TWO));
...
test("two notes with no shared root are reported as uncertain", () => {
  ...
  // 45/32 against the root: famously rootless, and here genuinely so.
...
test("memory fades, and an old note stops counting", () => {
  ...
  sonority.noteOn(1, THREE_OVER_TWO, { velocity: 1, at: 0 });
...
test("a note that has ended still colours what comes next", () => {
  ...
  sonority.noteOn(3, THREE_OVER_TWO, { at: 0.3 });
...
test("drift measures how far the centre has left home", () => {
  ...
  // A centre 3/2 above home is reported as 3/4 below it — the same
...
test("drift reports the shorter way round", () => {
  const sonority = new Sonority();
  // 4/3 up is the same as 3/2 down.
...
test("gravity pulls the centre toward home", () => {
  const notes = [[9, 8], [45, 32], [27, 16]]; // a triad two 3/2s from home
...
test("direction says which way the music has been walking", () => {
  const sonority = new Sonority();
  // Four steps up by 3/2 each time.
...
test("two sonorities over the same notes can disagree", () => {
  ...
  assert.ok(equals(upperOnly.read(0).centre, FIVE_OVER_FOUR), format(upperOnly.read(0).centre));
});
...
test("it survives a long stream of notes without growing", () => {
  ...
  for (let i = 0; i < 500; i++) {
    sonority.noteOn(i, i % 2 ? THREE_OVER_TWO : FIVE_OVER_FOUR, { at: i * 0.1 });
